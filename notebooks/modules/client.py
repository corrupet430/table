...
class OffVarianceClient:
    ...
    RESOURCE_MATCHES_FINISHED = 'finished'
    RESOURCE_MATCHES_UNFINISHED = 'unfinished'
    RESOURCE_EVENTS_FINISHED = 'finished-events'
    RESOURCE_ODDS_UNFINISHED = 'unfinished-odds'
    RESOURCE_MATCHES_FINISHED_URI = '/csv/matches/finished/'
    RESOURCE_EVENTS_FINISHED_URI = '/csv/events/finished/'
    RESOURCE_MATCHES_UNFINISHED_URI = '/csv/matches/future/'
    ...
    def get_finished_games(self, use_cache = False):
        if not use_cache:
            local_modification_date = self.get_modification_date(self.RESOURCE_MATCHES_FINISHED)
            modification_date = self.request_modification_date(self.RESOURCE_MATCHES_FINISHED_URI)

            if local_modification_date != modification_date:
                response = self.request_data(self.RESOURCE_MATCHES_FINISHED_URI)
                self.set_modification_date(self.RESOURCE_MATCHES_FINISHED, response.modification_date)
                self.store_data(self.RESOURCE_MATCHES_FINISHED, response.data)

        return self.read_data(self.RESOURCE_MATCHES_FINISHED)

    def get_unfinished_games(self, use_cache = False):
        if not use_cache:
            local_modification_date = self.get_modification_date(self.RESOURCE_MATCHES_UNFINISHED)
            modification_date = self.request_modification_date(self.RESOURCE_MATCHES_UNFINISHED_URI)

            if local_modification_date != modification_date:
                response = self.request_data(self.RESOURCE_MATCHES_UNFINISHED_URI)
                self.set_modification_date(self.RESOURCE_MATCHES_UNFINISHED, response.modification_date)
                self.store_data(self.RESOURCE_MATCHES_UNFINISHED, response.data)

            odds_local_modification_date = self.get_modification_date(self.RESOURCE_ODDS_UNFINISHED)
            ...
            if odds_local_modification_date != odds_modification_date:
                ...
                self.set_modification_date(self.RESOURCE_ODDS_UNFINISHED, response.modification_date)
                self.store_data(self.RESOURCE_ODDS_UNFINISHED, response.data)

        main_df = self.read_data(self.RESOURCE_MATCHES_UNFINISHED)
        odds_df = self.read_data(self.RESOURCE_ODDS_UNFINISHED)

        return main_df.merge(odds_df, on='id', suffixes=('_old',''))

    def get_finished_events(self, use_cache = False):
        if not use_cache:
            local_modification_date = self.get_modification_date(self.RESOURCE_EVENTS_FINISHED)
            modification_date = self.request_modification_date(self.RESOURCE_EVENTS_FINISHED_URI)

            if local_modification_date != modification_date:
                response = self.request_data(self.RESOURCE_EVENTS_FINISHED_URI)
                self.set_modification_date(self.RESOURCE_EVENTS_FINISHED, response.modification_date)
                self.store_data(self.RESOURCE_EVENTS_FINISHED, response.data)

        return self.read_data(self.RESOURCE_EVENTS_FINISHED)
